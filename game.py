...
def terrain_height_tile(tile_x: int, tile_z: int, seed: int, step: int) -> float:
    """Cached terrain sampling for render tiles to avoid recomputing noise every frame."""
    wx = tile_x * step
    wz = tile_z * step
    return terrain_height(wx, wz, seed)
...
class VoxelSkywaysGame:
    def __init__(self, width: int, height: int, headless: bool = False):
        ...
        self.active_render_distance = 26
    ...
    def _handle_keydown(self, key: int) -> None:
        ...
        if key == pygame.K_LEFT:
            ...
            self.active_render_distance = min(self.active_render_distance, self.render_distance)
    ...
    def _auto_tune_render_distance(self) -> None:
        if self.menu_open or not self.started:
            return

        # If the user asks for higher render distance than the machine can hold,
        # adaptively step down to keep flight responsive.
        if self.fps < 35:
            self.low_fps_streak += 1
            self.high_fps_streak = 0
            if self.low_fps_streak >= 18:
                self.active_render_distance = max(10, self.active_render_distance - 1)
                self.low_fps_streak = 0
        elif self.fps > 72 and self.active_render_distance < self.render_distance:
            self.high_fps_streak += 1
            self.low_fps_streak = 0
            if self.high_fps_streak >= 36:
                self.active_render_distance += 1
                self.high_fps_streak = 0
        else:
            self.low_fps_streak = 0
            self.high_fps_streak = 0
        ...
        self.active_render_distance = min(self.active_render_distance, self.render_distance)
        self.active_render_distance = max(10, self.active_render_distance)
    ...
    def _draw_world(self) -> None:
        ...
        step = 6
        rd = self.active_render_distance
        base_cx = int(self.pos_x // step)
        base_cz = int(self.pos_z // step)

        # Draw far-to-near rings with adaptive sampling stride to reduce per-frame work.
        for ring in range(rd, -1, -1):
            for tz in range(-ring, ring + 1):
                for tx in range(-ring, ring + 1):
                    if abs(tx) != ring and abs(tz) != ring:
                        continue

                    dist = max(abs(tx), abs(tz))
                    stride = 1 if dist <= 10 else (2 if dist <= 18 else 3)
                    if tx % stride or tz % stride:
                        continue
                    ...
                    wx = tile_x * step
                    wz = tile_z * step
                    h = terrain_height_tile(tile_x, tile_z, self.seed, step)
                    ...
                    pygame.draw.rect(self.screen, shaded, (int(sx), int(sy), size, size))
    ...
    def _draw_menu(self) -> None:
        ...
        lines = [
            ...
            f"Adaptive active distance: {self.active_render_distance}",
    ...
    def run(self, max_frames: int | None = None) -> int:
        ...
        while self.running:
            ...
            self._update(dt)
            self._auto_tune_render_distance()
